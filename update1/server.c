...
char *makeResMessage(RES_TYPE type,SIGNIU_RES res,char *nickname)
{
						...
						else
						{
							switch(buff[0]-'0')
							{
								case REGISTER:
								{
									...
									if(ruler == 0)
									{
										send(fds[i],makeResMessage(REG_RES,EXISTED,nickname),25,0);
									}
									else {
										send(fds[i],makeResMessage(REG_RES,SUCCESS_SIGNUP,nickname),25,0);
									}
									break;
								}
								case LOGIN:
								{
									...
									if(ruler == 0)
									{
										send(fds[i],makeResMessage(LOG_RES,NOT_EXIST,nickname),25,0);
									} else if(ruler == 1)
									{
										send(fds[i],makeResMessage(LOG_RES,WRONG_PASS,nickname),25,0);
									}else send(fds[i],makeResMessage(LOG_RES,SUCCESS_SIGNIN,nickname),25,0);
									break;
								}
