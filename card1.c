...
void newRoom(int status,char* password,Player creator_room)
{
...
{
	Player new =(Player)calloc(1,sizeof(struct gamer));
...
void showPlayer()
{
	...
	while(p!=NULL)
	{
		printf("Slot%d, Player %s: %d$\n",p->slot,p->nickname,p->money );
		p=p->next;
	}
}
int* rollcard()
...
void match()
{
	...
	while(p!=NULL)
	{
		drawHand(p->nickname,playercard[count]);
		p->hand = calculate(playercard[count]);
		printf("Rank: %d|High: %d|Second: %d|Third: %d|Fouth: %d|Last: %d\n", p->hand.rank,p->hand.high_value,p->hand.second_value,p->hand.third_value,p->hand.fouth_value,p->hand.last_value);
		count++;
		p=p->next;
	}
	rank = ranking();
...
{
	Player p1 = newPlayer("Bao",20000);
	Player p2 = newPlayer("Du",20000);
	Player p3 = newPlayer("Nghia",40000);
	newRoom(0,"",p1);
	joinRoom(p2);
	joinRoom(p3);
	Player p;
